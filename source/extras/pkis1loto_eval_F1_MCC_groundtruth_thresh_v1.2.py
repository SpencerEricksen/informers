...
for f in rankings_files:
    ...
    for targ in df_rankings.columns:
        if df_rankings[targ].count() < 300:
            ...
            # for binary predictions assign 'active' label to N top 
            # ranking cpds where N is the experimentally known number of true actives on target
...
df_f1.to_csv('../output_pkis1loto/metrics/pkis1loto_eval_F1_groundtruth_thresh_v1.2.csv', index_label='target')
df_mcc.to_csv('../output_pkis1loto/metrics/pkis1loto_eval_MCC_groundtruth_thresh_v1.2.csv', index_label='target')
